...
//#define   GET_WIDTH(layer)        ((layer)->width + 4-( (((layer)->width-1)%4)
//+1)) #define   GET_WIDTH(layer)        ((layer)->width%4==0 ? (layer)->width :
//layer->width + 4-(layer->width%4)) #define   GET_WIDTH(layer) ((layer)->width)
#define GET_WIDTH(image)                                                       \
  ((image)->rowbytes /                                                         \
   sizeof(PixelType)) // rowbytes = 横のバイト数 1ピクセル4バイトなので /4
...
#define GET_SIGN(a)                                                            \
  ((a) / ABS((a))) // 符号を得る マイナスだったら -1、プラスだったら+1
...
//          ratio:          ブレンドする割合。1.0fでblend_targetのまんまになる
//          0.0fでref
//----------------------------------------------------------------------------//
template <typename PixelType>
