...
void downMode_LeftCountLength(BlendingInfo<PixelType> *info) {
  ...
  while (1) {
    count_target =
        info->in_target - (len - 1); // Change the target for examination

    // Check if it's a solid fill
    if (ComparePixel(count_target, count_target - 1)) { // Scan from the bottom

      info->core[0].start =
          (float)(info->i + 1); // Image coordinates start from top-left (0,0),
                                // but logical lines start from right
    ...
    count_target = info->in_target + width -
                   (len - 1); // Change the target for examination ******

    if (ComparePixel(count_target, count_target - 1)) { // Scan from the top
      info->core[0].start =
          (float)(info->i + 1); // Image coordinates start from top-left (0,0),
                                // but logical lines start from right
      info->core[0].end = (float)(info->i + 1) - (float)len;
      ...
      if (width - 2 > info->i && info->i > 2 && height - 2 > info->j &&
        ...
        // Examine the length of the corner above the line, and if the
        // difference is 1, make a correction
                             ...
                             width)) // Check whether it's a corner **********
        {
          ...
          if (sc_info.core[0].length - len == 1) {
            info->core[0].end -=
                0.5f; // Correct for something half a pixel to the left
          }
        }
      }
...
void downMode_RightCountLength(BlendingInfo<PixelType> *info) {
  ...
  count_target =
      info->in_target - width; // Change the target for examination ******
  ...
  while (1) {
    ...
    count_target = info->in_target - width +
                   len; // Change the target for examination *********
    ...
    if (ComparePixel(count_target, count_target + 1)) {
      ...
      if (width - 2 > info->i && info->i > 2 && height - 2 > info->j &&
          info->j > 2) {
        // Check if end value correction is needed
        // Examine the length of the corner below the line, and if the
        // difference is 1, make a correction Check if it's a corner
        if (!(info->flag & SECOND_COUNT) &&
          ...
          if (len - sc_info.core[1].length == 1 &&
            ...
            info->core[1].end -=
                0.5f; // Correct for something half a pixel to the right
          }
        }
      }
...
void downMode_TopCountLength(BlendingInfo<PixelType> *info) {
  ...
  while (1) {
    count_target = info->in_target -
                   (len - 1) * width; // Change the target for examination ****
    ...
    count_target = info->in_target - (len - 1) * width +
                   1; // Change the target for examination ***
...
void downMode_BottomCountLength(BlendingInfo<PixelType> *info) {
  ...
  while (1) {
    count_target = info->in_target +
                   (len * width); // Change the target for examination *****
    ...
    count_target = info->in_target + (len * width) -
                   1; // Change the target for examination ****
    ...
    if (ComparePixel(count_target, count_target + width)) // *********
    {
      ...
      if (width - 2 > info->i && info->i > 2 && height - 2 > info->j &&
        ...
        // Check the length of the angle one pixel below the line, if the
        // difference is one, correct it Check if it's an angle
        if (!(info->flag & SECOND_COUNT) &&
            ComparePixel(count_target, count_target + 1)) {
          // Count here //
          BlendingInfo<PixelType> sc_info;
...
void downMode_LeftBlending(BlendingInfo<PixelType> *info) {
  ...
  // Normally, the Length value is halved for use, but in this case, the unit is
  // 1/2 pixel, so it's doubled. len*(1/2)*2 = len, so it can be used directly
#if 0
    if(flg & CR_FLG_FILL)
    {
        //----------------------//
...
#endif
  {
    ...
    // Base × height ÷ 2 = Base × ((Base / Total Base) × Total Height) ÷ 2 = l *
    // l * 0.5 * 0.5 / len Calculated from the left
...
void downMode_RightBlending(BlendingInfo<PixelType> *info) {
...
#endif
  {
    ...
    if (length > 0) {
      ...
      // This end_p is needed? The calculation of the ratio doesn't depend on
      // coordinate values, so it might not be necessary? Referring to link 8
      end_p = (int)(end - 0.000001); // A workaround for exact values like 4.0f
                                     // causing issues
...
void downMode_TopBlending(BlendingInfo<PixelType> *info) {
  ...
  { // Normal
    ...
    for (t = 0; t < blend_count; t++) {
      l = (float)(end_p + 1 + t) -
          end; // Calculate the base length for each pixel blend // ****
      ratio = (l * l * 0.5f * 0.5f) /
              len; // Calculate the area ratio for each pixel blend
      ...
      blend_target +=
          in_width; // Move to the next pixel in the input buffer // ******
      out_target +=
          out_width; // Move to the next pixel in the output buffer // ******
    }
  }
}
...
void downMode_BottomBlending(BlendingInfo<PixelType> *info) {
  ...
  // Normally, the Length value is halved, but in this case, since the unit is
  // half-pixel units, it's doubled, len*(1/2)*2=len, so we can use it as is.
  ...
  { // Normal
    if (length > 0) {
      ...
      end_p = (int)(end - 0.00001); // A workaround for cases like 4.0f where it
                                    // becomes incorrect
      ...
      for (t = 0; t < blend_count; t++) {
        l = end - (float)(end_p - t); // Calculate the base length for each
                                      // pixel blend // *****
        ratio = (l * l * 0.5f * 0.5f) /
                len; // Calculate the area ratio for each pixel blend
...
template void
downMode_LeftCountLength<PF_Pixel8>(BlendingInfo<PF_Pixel8> *info);
template void
downMode_LeftCountLength<PF_Pixel16>(BlendingInfo<PF_Pixel16> *info);

template void
downMode_RightCountLength<PF_Pixel8>(BlendingInfo<PF_Pixel8> *info);
template void
downMode_RightCountLength<PF_Pixel16>(BlendingInfo<PF_Pixel16> *info);

template void downMode_TopCountLength<PF_Pixel8>(BlendingInfo<PF_Pixel8> *info);
template void
downMode_TopCountLength<PF_Pixel16>(BlendingInfo<PF_Pixel16> *info);

template void
downMode_BottomCountLength<PF_Pixel8>(BlendingInfo<PF_Pixel8> *info);
template void
downMode_BottomCountLength<PF_Pixel16>(BlendingInfo<PF_Pixel16> *info);
...
template void
downMode_RightBlending<PF_Pixel16>(BlendingInfo<PF_Pixel16> *info);
...
template void
downMode_BottomBlending<PF_Pixel16>(BlendingInfo<PF_Pixel16> *info);
